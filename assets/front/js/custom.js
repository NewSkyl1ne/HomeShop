...
// Wishlist Section 
    ...
    $(document).on('click', '.add-to-wish', function(){
        $.get( $(this).data('href') , function( data ) {
            ...
            if(data == 1) {
                $.notify(gs.add_wish,"success");
              }
            else {
                $.notify(gs.already_wish,"error");
              }
...
// Compare Section 
    ...
    $(document).on('click', '.add-to-compare', function(){
        $.get( $(this).data('href') , function( data ) {
            ...
            if(data[0] == 0) {
                $.notify(gs.add_compare,"success");
              }
            else {
                $.notify(gs.already_compare,"error");
              }
...
// Cart Section 
    ...
    $(document).on('click', '.add-to-cart', function(){
        ...
        $.get( $(this).data('href') , function( data ) {
            if(data == 'digital') {
                $.notify(gs.already_cart,"error");
             }
            else if(data == 0) {
                $.notify(gs.out_stock,"error");
              }
            else {
              $("#cart-count").html(data[0]);
              //$("#cart-items").load(mainurl+'/carts/view');
              $.notify(gs.add_cart,"success");
              }
        });
                    return false;
    });
...
// CANCEL CLICK ENDS
    ...
    $(document).on("click", "#addcrt" , function(){
     var qty = $('.qttotal').val();//$('.qttotal').html();
        ...
        var total = $("#grandtotal").val();
            $.ajax({
                    ...
                    success:function(data){
                        ...
                        {
                            $.notify(gs.already_coupon,"error");
                            $("#code").val("");
                        }
                        else
                        {
       ...
       $(document).on("change", ".color" , function(){
        ...
        $(this).css('background',colors);
            $.ajax({
                    ...
                    success:function(data){
                        $.notify("Successfully Changed The Color","success");
                      }
              }); 
       });
