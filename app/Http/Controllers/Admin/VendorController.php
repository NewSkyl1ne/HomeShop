...
class VendorController extends Controller
{
	    ...
	    public function datatables()
	    {
	        $datas = User::where('is_vendor','=',2)->orWhere('is_vendor','=',1)->orderBy('id','desc')->get();
	         ...
	         return Datatables::of($datas)
                ...
                '<option value="'. route('admin-vendor-st',['id1' => $data->id, 'id2' => 2]).'" '.$s.'>Active</option>'.
                '<option value="'. route('admin-vendor-st',['id1' => $data->id, 'id2' => 1]).'" '.$ns.'>Deactive</option></select></div>';
                                }) 
	                            ->addColumn('action', function(User $data) {
	                                return '<div class="action-list"><a href="' . route('admin-vendor-show',$data->id) . '" > <i class="fas fa-eye"></i> View Details</a><a data-href="' . route('admin-vendor-edit',$data->id) . '" class="edit" data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit"></i>Edit</a><a href="javascript:;" class="send" data-email="'. $data->email .'" data-toggle="modal" data-target="#vendorform"><i class="fas fa-envelope"></i> Send Message</a><!--<a href="javascript:;" data-href="' . route('admin-vendor-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a>--></div>';
	                            }) 
    ...
    public function subsdatatables()
    {
         ...
         $datas = UserSubscription::join('users', 'users.id', '=', 'user_id')->where('user_subscriptions.status',1 )->orderBy('user_subscriptions.id','desc')->get();
        //  var_dump($datas);
         //--- Integrating This Collection Into Datatables
         return Datatables::of($datas)
                            ...
                            ->addColumn('action', function(UserSubscription $data) {
                                return '<div class="action-list"><a data-href="' . route('admin-vendor-sub',$data->id) . '" class="view" data-toggle="modal" data-target="#modal1"> <i class="fas fa-eye"></i>View Details</a></div>';
                            }) 
                            ->rawColumns(['action'])
                            ->toJson(); //--- Returning Json Data To Client Side
    ...
    public function update(Request $request, $id)
    {
        ...
        $data = $request->all();
        if($file = $request->file('cr_doc'))
        {
        $name = time().$file->getClientOriginalName();
        $file->move('assets/images/vendorCRDocs',$name);
        $data['cr_doc'] = $name;
        }
        if($file = $request->file('doc'))
        {
        $name = time().$file->getClientOriginalName();
        $file->move('assets/images/vendorDocs',$name);
        $data['doc'] = $name;
        }
        $user->update($data);
        ...
        public function withdrawdatatables()
        {
             ...
             return Datatables::of($datas)
                                ...
                                ->addColumn('action', function(Withdraw $data) {
                                    $action = '<div class="action-list"><a data-href="' . route('admin-vendor-withdraw-show',$data->id) . '" class="view" data-toggle="modal" data-target="#modal1"> <i class="fas fa-eye"></i> View Details</a>';
