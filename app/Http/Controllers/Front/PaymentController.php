 ...
 public function store(Request $request){
    ...
    // Redirect to paypal IPN
     ...
     public function payreturn(){
        $mainorder = Auth::user();
        $url = url('/');
        $orderlist=DB::table('orders')->where('user_id','=',$mainorder->id)->latest()->first();
        $urlextend='/user/order/'.$mainorder->id;
        // $email=$orderlist->customer_email;
        // $email2=$orderlist->shipping_email;
        // $to=$email;//$email;
        // //$to2=$email2;
        // $subject = 'Your order '.$orderlist->order_number.' is Processing!';
        // $body = "Hello ".$orderlist->customer_name.","."\n Thank you for shopping with us. We are looking forward to your next visit."."Visit this to View Your Order.".$url.$urlextend."";
        // \App\Helpers\Helper::UserEmail($to,$subject,$body);
        $order = DB::table('orders')->where('order_number','=',$orderlist->order_number)->get()->first();
        $vendor=db::table('users')->join('vendor_orders','vendor_orders.user_id','=','users.id')->where('vendor_orders.order_number',$order->order_number)->get()->first();  
